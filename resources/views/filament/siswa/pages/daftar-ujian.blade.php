                ...
                <div
                    ...
                    <div class="p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="w-full sm:w-auto">
                            <h3
                                class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white flex items-center gap-3">
                                {{ $event->nama_ujian }}
                            </h3>
                            <div class="mt-1 sm:mt-2 text-sm text-gray-600 dark:text-gray-400 space-y-1">
                                ...
                                @if ($event->is_registration_closed)
                                    <span
                                        class="inline-flex items-center mt-1 px-3 py-1 text-xs font-semibold bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-200 rounded-full">
                                        🔴 Pendaftaran Ditutup
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center mt-1 px-3 py-1 text-xs font-semibold bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-200 rounded-full">
                                        🟢 Pendaftaran Dibuka
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                            @if ($isRegistered)
                                @if ($isLulus)
                                    <span
                                        class="inline-flex justify-center px-4 py-2 text-sm font-medium text-green-800 bg-green-100 dark:bg-green-900 dark:text-green-100 rounded-full text-center">
                                        🎉 Kamu sudah lulus
                                    </span>
                                @else
                                    <span
        ...
        @if ($showVerification)
            <div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center px-3 sm:px-0">
                ...
                <div
                    ...
                    {{-- HEADER --}}
                    <div
                        class="flex justify-between items-center px-5 sm:px-6 py-4 bg-gradient-to-r from-primary-600 to-primary-700 text-white">
                        <h3 class="text-lg sm:text-xl font-semibold flex items-center gap-2">✏️ Verifikasi & Edit Data
                            Siswa</h3>
           ...
           text-gray-900 dark:text-gray-100">
                            ...
                            {{-- Tempat Lahir --}}
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Tempat
                                    Lahir</label>
                            ...
                            {{-- Sabuk Saat Ini --}}
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Sabuk
                                    Saat Ini</label>
                    ...
                    {{-- FOOTER --}}
                    <div
                        class="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4 px-5 sm:px-8 py-5 bg-gray-100 dark:bg-gray-800 border-t border-gray-300 dark:border-gray-700">
